...
(def opts
  [["-h" "--help" "Print this help message and exit."]
   ...
   ["-u" "--adjust" "Adjust the library panel (PANEL_UID) using the DATASOURCE_UIDS as targets of a grafana-instance (GRAFANA_URL and GRAFANA_TOKEN)."]
...
(defn print-usage [opts-map]
  (do (println "Usage Examples:\n")
      (println "active-grafana.main-adjust -- --help")
      (println "active-grafana.main-adjust -- --show   --url=<grafana-url> --token=<grafana-token>")
      (println "active-grafana.main-adjust -- --adjust --url=<grafana-url> --token=<grafana-token> --panel-uid=<panel-uid> --datasource-uids=\"<datasource-uid-1> <datasource-uid-2> ... <datasource-uid-n>\"")
      (println "\nOptions:")
